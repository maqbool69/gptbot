...
# Define constants
TOP_BOT_IMAGES = r'C:\Users\Hp\Desktop\Top bot images'
...
async def generate_prompt_and_submit_command(page, prompt: str):
    try:
        ...
        logger.exception(f"An error occurred while submitting the prompt: {e}")
        raise e
...
def gpt3_midjourney_prompt(prompt: str, engine='text-davinci-003', temp=0.7, top_p=1.0, tokens=400, freq_pen=0.0, pres_pen=0.0) -> str:
    ...
    try:
        ...
        logger.exception(f"Error occurred: {e} while generating prompt.")
        raise e
    
@eel.expose
def gpt3_midjourney_prompt_to_generate_title(prompt: str) -> str:
    """
    Function to generate a title using the OpenAI GPT-3 model.
    ...
    try:
        ...
        logger.exception(f"Error occurred: {e} while generating title.")
        raise e
    
@eel.expose
def gpt3_midjourney_prompt_to_generate_keywords(prompt: str) -> list:
    """
    Function to generate keywords using the OpenAI GPT-3 model.
    ...
    try:
        ...
        logger.exception(f"Error occurred: {e} while generating keywords.")
        raise e
...
async def get_last_message(page) -> str:
    ...
    try:
        ...
        logger.exception(f"Error occurred: {e} while getting the last message.")
        raise e
...
async def main(bot_command: str, channel_url: str, PROMPT: str):
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # Keep trying until credentials are successfully read
            while True:
                try:
                    with open("credentials.txt", "r") as f:
                        ...
                        if email and password:
                            ...
                            logger.info("Waiting for credentials to be added to credentials.txt. Retrying in 5 seconds...")
                            # Wait for 5 seconds before retrying
                            await asyncio.sleep(5)
            
                except Exception as e:
                    logger.exception(f"Error occurred: {e} while reading credentials.txt.") 
                    logger.info("Waiting for credentials.txt to be created. Retrying in 5 seconds...")
                    # If credentials.txt doesn't exist or there's another error, wait for it to be corrected
                    await asyncio.sleep(5)
        ...
        logger.exception(f"Error occurred: {e} while executing the main function.")
        raise e
    finally:
        if browser:
            await browser.close()
...
async def open_discord_channel(page, channel_url: str, bot_command: str, PROMPT: str):
    ...
    try:
        ...
        logger.exception(f"An error occurred while opening the channel and entering the bot command: {e}")
        raise e
...
async def select_upscale_option(page, option_text: str):
    ...
    try:
        ...
        logger.exception(f"An error occurred while selecting the upscale option: {e}")
        raise e
        ...
        while True:
            ...
            if 'U1' in last_message:
                ...
                try:
                    ...
                    logger.exception(f"An error occurred while selecting upscale options: {e}")
                    raise e
...
def check_api_key(api_key: str) -> str:
    ...
    try:
        ...
        logger.exception(f"An error occurred while checking or setting the OpenAI API key: {e}")
        raise e
...
def initialize_bot(api_key: str):
    ...
    try:
        ...
        logger.exception(f"An error occurred while initializing the bot: {e}")
        raise e
